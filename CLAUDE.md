...
| `token:create` / `token:drag` / `token:move` / `token:update` / `token:delete` / `token:select` | Token CRUD + real-time sync (includes darkvision/speed) |
...
In `client/src/hooks/useSocket.ts`: `room:players`, `chat:message`, `chat:whisper`, `dice:roll`, `dice:roll_private`, `map:fog:update`, `combat:start`, `combat:next_turn`, `combat:prev_turn`, `combat:end`, `combat:add`, `combat:remove`, `combat:initiative:update`
