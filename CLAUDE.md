...
- **Combat recording system**: 3-phase lifecycle — **setup** (auto-import all map tokens, DM adjusts initiative/name), **active** (round-by-round logging, start/pause/resume), **end** (save to map history or discard). One combat per map. Auto-named "MapName-战斗X-Time". Combat records persisted per-map via REST API.
...
- **Player permissions**: players can create characters (+ Create button), place own tokens (auto-set ownerId), drag/edit/delete own tokens (HP, hide/show, context menu), link tokens to characters. Server-side ownership enforcement on token PATCH/DELETE (403 for non-owner non-DM). Players see combat sidebar when active, can interact with own tokens in combat.
- **Combat history**: completed combats stored per map, queryable via `GET /api/combat/map/:mapId/history`
...
| `combat:start` / `combat:start_recording` / `combat:pause` / `combat:end` / `combat:next_turn` / `combat:prev_turn` / `combat:add` / `combat:remove` / `combat:initiative:update` / `combat:update_name` | Combat lifecycle (3-phase: setup→active→end) |
| `combat:action` | Combat action with optional HP delta |
...
| Combat route | `server/src/routes/combat.ts` |
